...
const add_review = () => {
  http.post('/add_review', formData)
    .then((response) => {
      console.log('Add review success', response.data);
      router.push('/admin/reviews/list');
    })
    .catch((error) => {
      console.error('Adding review failed', error.response?.data);
      errorMessage.value = error.response?.data?.message || "Adding review failed. Please try again.";
    });
};
</script>
